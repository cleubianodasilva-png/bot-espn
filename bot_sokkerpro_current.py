...
# ═══════════════════════════════════════════════════════════════════════════════
# API 3 — ODDS: favorito pela menor odd
# ═══════════════════════════════════════════════════════════════════════════════
...
def get_favorito_odds(home, away, fid=None, league=None):
    """Retorna ('h'|'a', odd_h, odd_a) baseado na menor odd. Usa SokkerPro ou apifootball."""
...
def get_odd_favorito_num(home, away, fid=None, league=None, fid_raw=None):
    """Retorna a odd decimal do favorito (numero). Usa SokkerPro ou Odds API."""
    if fid_raw:
        try:
            headers = {}  # SokkerPro
            if r.status_code == 200:
...
def checar_resultado(sinal):
    ...
    try:
        eid_raw = str(sinal.get("fixture_id", ""))
        mercado = sinal.get("mercado")
        
        # 1. Busca detalhes do evento via SokkerPro
        data = _get_data()
        ...
        for cat in data['data']['sortedCategorizedFixtures']:
            for fix in cat['fixtures']:
                if str(fix.get('fixtureId', '')) == eid_raw:
                    ev = fix
                    break
            if ev: break
        ...
        is_final = (status in ["Finished", "After Pen.", "Awarded"])
        is_2h    = (status in ["2nd", "FTP"])
        
        if not (is_final or (mercado in ["HT", "LIMITEHT", "CORNER_HT"] and is_2h)):
            return None

        # Placar Final
        gh = int(ev.get("scoresLocalTeam", 0) or 0)
        ga = int(ev.get("scoresVisitorTeam", 0) or 0)
        total_final = gh + ga

        # Placar HT — SokkerPro retorna scoresHT como soma total, não individual
        # Vamos usar o scoresHT como total_ht mesmo
        scores_ht = str(ev.get("scoresHT", "0") or "0")
        if scores_ht.isdigit():
            total_ht = int(scores_ht)
        else:
            # Fallback: se vier "1-0" ou similar
            partes = scores_ht.split("-")
            if len(partes) == 2:
                total_ht = int(partes[0]) + int(partes[1])
            else:
                total_ht = 0
        ...
        if mercado in ["HT", "LIMITEHT"]:
            ...
            c_final = int(ev.get("localCorners", 0) or 0) + int(ev.get("visitorCorners", 0) or 0)
